 ...
 * tail for snapshots, and opens a temp file when the full output needs to be
 * preserved.
 */
export class OutputAccumulator {
  private readonly maxLines = DEFAULT_MAX_LINES;
  private readonly maxBytes = DEFAULT_MAX_BYTES;
  private readonly maxRollingBytes = this.maxBytes * 2;
  ...
  private trimTail(): void {
    ...
    while (
      start < buffer.length &&
      ((buffer[start] as number) & 0xc0) === 0x80
    ) {
      start++;
    }
  ...
  private getSnapshotText(): string {
    ...
    // The trimmed tail starts mid-line; drop the partial first line.
  ...
  private ensureTempFile(): void {
    ...
    this.tempFilePath = join(
      tmpdir(),
      `infrawiki-bash-${randomBytes(8).toString("hex")}.log`,
    );
